# Script to implement the CART-like DT classifier from
# Chapter 7 of "Programming Collective Intelligence" by
# T. Segaran, O'Reilly, (c) 2007
#
# The Python Imaging Libraray (PIL) is needed to generate
# pngs of the derived decision trees.
#
#
...
# First, compute the probability of each possible outcome by dividing the
# number of times that outcome occurs by the total number of rows (records)
# in the dataset. Then, add up the products of all these probabilities.
# This yields the overall chance that a row (record) would be randomly
# assigned to the wrong outcome; the higher the prob., the worse the split.
...
# A measurement of how different the outcomes are from each other; the smaller
# the entropy, the better the splitting.
...
# For pruning, we check a pair of nodes with a common parent and
# see if merging them would INCREASE the entropy by less than
# "mingain".  If so, those leaf nodes are merged into a single
# node with all possible outcomes.  This helps to avoid overfitting.
#
